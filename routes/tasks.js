...
import { sendPushNotification, createNotification, getNotificationsForUser } from '../firebase.js'
...
router.post('/:id/request-reopen', async (req, res) => {
  try {
    ...
    // Créer les notifications en base de données pour les administrateurs
    const admins = await User.find({ role: 'admin' })
    const dbNotifPromises = admins.map(admin =>
      createNotification({
        forAdmin: true,
    ...
    await Promise.all(dbNotifPromises)

    // Send Firebase push notifications to all admins with FCM tokens
    const adminUsers = await User.find({ role: 'admin', fcmToken: { $exists: true, $ne: null } })
    const adminNotifPromises = adminUsers.map(adminUser =>
      sendPushNotification(
        adminUser.fcmToken,
        'Reopen Request',
        `${req.user.name} a demandé la réouverture de la tâche "${task.title}".`,
        { taskId: task._id.toString(), type: 'REOPEN_REQUEST', recipientId: adminUser._id.toString() }
      )
    )
...
router.post('/:id/comments', async (req, res) => {
  try {
    ...
    // Créer les notifications en base de données pour tous les participants (sauf l'auteur)
    const dbNotifPromises = Array.from(participantIds).map(pId =>
      createNotification({
    ...
    // Send Firebase push notifications to participants (excluding the author)
    const participants = await User.find({
      _id: { $in: Array.from(participantIds) },
      fcmToken: { $exists: true, $ne: null }
    })
    const participantNotifPromises = participants.map(p =>
      sendPushNotification(
        p.fcmToken,
        'New Comment',
        `${req.user.name} a répondu dans la tâche "${taskName}".`,
        { taskId: task._id.toString(), type: 'COMMENT', recipientId: p._id.toString() }
      )
    )
    await Promise.all(participantNotifPromises)
